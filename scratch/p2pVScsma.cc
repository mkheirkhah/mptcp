...
uint32_t LinkRate = 100000000; // 100Mbps
uint32_t Delay = 2; // RTT: 4ms
Time cDelay = MilliSeconds(Delay);
double LossRate = 0.0;
double dtq = 100;
uint32_t segSize = 536;
static const uint32_t totalTxBytes = 5000000; // 5MB
...
// Creation of the hosts
  ...
  if (p2p)
    ...
    {// CSMA
      ...
      //
      netDevices = csma_hub_0.Install(nodes);
    }
